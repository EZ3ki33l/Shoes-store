        ...
        {mainCategories.map((category) => (
          <li key={category.id} className="flex flex-col gap-2">
            ...
            {category.children.length > 0 && (
              <ul className="ml-6 flex flex-col gap-1 border-l-2 border-gray-200 pl-4">
                {category.children.map((child) => (
                  <li
                    key={child.id}
                    className="group flex items-center gap-3 rounded border-dashed bg-gray-50 p-2"
                  >
                    <Typo variant="body" className="flex-1 text-sm text-gray-600">
                      {child.name}
                    </Typo>
