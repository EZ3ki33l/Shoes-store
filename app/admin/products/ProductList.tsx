...
/**
 * Liste client des produits : recherche, groupement par marque (repliable),
 * résumé des couleurs / stocks et modal de prévisualisation.
 */
...
/**
 * Ordonne les images pour la prévisualisation d'une couleur :
 * couleur ciblée → génériques (color null) → autres couleurs.
 */
function imagesForColor(product: Product, color: string) {
  ...
  const other = product.images.filter((img) => img.color != null && img.color !== color)
  const combined = [...forColor, ...generic, ...other]
  return combined.length > 0 ? combined : product.images
}

function ProductPreviewModal({ preview, onClose }: { preview: PreviewState; onClose: () => void }) {
  const { product, color } = preview
  const images = imagesForColor(product, color.color)

  // Escape pour fermer + bloque le scroll du body tant que la modal est ouverte
  useEffect(() => {
  ...
  // Portal vers document.body pour sortir du stacking context de la liste
  return createPortal(
    <div
    ...
    >
      <div
        ...
        className="bg-card relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border p-5 shadow-lg"
      ...
      >
        ...
        {images.length > 0 ? (
          ...
          >
            {images.map((img, index) => (
              <div key={`${img.url}-${index}`} className="bg-muted overflow-hidden rounded border">
                <Image
                  src={img.url}
          ...
          <div className="bg-muted text-muted-foreground mb-4 flex h-48 items-center justify-center rounded border text-sm">
            Aucune image
          </div>
        )}
        ...
        <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
          ...
          <dd>{color.stock}</dd>
          <dt className="text-muted-foreground">Pointures</dt>
    ...
    return (
      <li key={product.id} className="overflow-hidden rounded border">
        ...
        {product.colors.length > 0 ? (
              ...
              <tbody>
                {product.colors.map((color) => (
                  <tr key={color.color} className="border-b last:border-b-0">
                    ...
                    <td className="text-muted-foreground px-3 py-2">{color.sizes.length}</td>
                    <td className="text-muted-foreground px-3 py-2">{color.stock}</td>
          ...
          <p className="text-muted-foreground px-3 py-2 text-sm">Aucune variante</p>
        )}
      </li>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {filteredProducts.length === 0 && (
        <Typo variant="body" className="text-muted-foreground">
          ...
          return (
            <div key={brand.id} className="flex flex-col gap-3">
              ...
              >
                ...
                <span className="text-muted-foreground text-sm">({brandProducts.length})</span>
              </button>
              {!isCollapsed && (
